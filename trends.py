...
def make_tweet_fn(text, time, lat, lon):
    ...
    tweet_information = {'text': text, 'time': time, 'latitude': lat, 'longitude': lon}
    def return_corresp_value(key):
        nonlocal tweet_information
        if key == 'text':
            return tweet_information['text']
        elif key == 'time':
            return tweet_information['time']
        elif key == 'lat':
            return tweet_information['latitude']
        elif key == 'lon':
            return tweet_information['longitude']
    return return_corresp_value
...
def extract_words(text):
    ...
    for letter in text:
        if letter in ascii_letters:
            str = str + letter
        else: #if letter is not in ascii_letters
            if str != "":
                words.append(str)
            str = ""
    if str != "": #covers cases when string doesnt end with non-ascii letter
        words.append(str)
    return words
